...
abalone_clean <- abalone |> 
  # cleaned column names with underscores and no caps
  clean_names()  |> 
  # select for columns of interest
  select(mean_p_h, change_in_area_mm2_d_1_25)  |> 
  # change column names
  rename(mean_ph = mean_p_h, # new_name = old_name
...
ggplot(data = abalone_clean, # data is abalone_clean
       aes(x = mean_ph, # x is mean ph (predictor var)
           y = change_in_area)) + # y is change in area (response var)
  # 
  geom_point(size = 4,
             stroke = 1,
             fill = "firebrick4",
...
**The missing value comes from tank B1B.**  
...
**With each increasing in pH, the model predicts an increase of 3.0 +/- 0.76 change in shell surface area per day (mm^2^ d^-1^).**  

What is the intercept estimate (with standard error?)  

**When pH = 0, the model predicts that abalone change in shell surace area per day is -18.5. (Note to self: this is not necessarily biologically realistic, but is something the model generates)**
...
**predicted is the mdoel predictions for change in shell surface area per day**
...
**This is a ggeffects output object and a data frame**  
